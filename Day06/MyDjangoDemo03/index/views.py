...
def add_views(request):
  ...
  # obj = Author.objects.create(name='陈正豪',age=32,email='chen@163.com')
  ...
  # book1 = Book(**dic)
  # book1.save()
  # Book.objects.create(title='平凡的世界', publicate_date='1995-10-10')
  # book=Book()
  # book.title='三体'
  # book.publicate_date='2010-12-10'
  # book.save()
  dic={
      'title':'墨菲定律',
      'publicate_date': '2019-10-10'
  }
  book2=Book(**dic)
  book2.save()
...
def query_views(request):
  ...
  print(result['avg'])
  result = Author.objects.all().aggregate(num=Count('age'))
  print(result['num'])
...
def update_views(request,id):
  ...
  # return HttpResponse('传递过来的id为：'+id)
...
def doF_views(request):
  Author.objects.all().update(age=F('age')+15)
  return redirect('/07-queryall')

def doQ_views(request):
  authors=Author.objects.filter(Q(age=33)| Q(age=21))
  for au in authors:
    print(au.name)
  return HttpResponse('OK')

def raw_views(request):
  sql="select * from index_author where age>26"
...
def titleCount_views(request):
  # count = Book.objects.title_count('王')
  # print(count)
